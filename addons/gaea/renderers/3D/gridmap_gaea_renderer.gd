...
func _ready() -> void:
	...
	if Vector2i(grid_map.cell_size.x * grid_map.scale.x, grid_map.cell_size.z * grid_map.scale.z) != generator.tile_size:
		...
		generator.tile_size = Vector2i(grid_map.cell_size.x * grid_map.scale.x, grid_map.cell_size.z * grid_map.scale.z)
			...
			for z in range(area.position.z, area.end.z + 1):
				...
				if erase_empty_tiles:
					...
					for layer in range(generator.grid.get_layer_count()):
						if generator.grid.has_cell(cell_2d, layer) and generator.grid.get_value(cell_2d, layer) != null:
							has_cell = true
							break
				...
				for layer in range(generator.grid.get_layer_count()):
					if only_draw_visible_cells and generator.grid.has_method("has_empty_neighbor") and not generator.grid.has_empty_neighbor(cell_2d, layer):
						continue

					var tile_info = generator.grid.get_value(cell_2d, layer)
